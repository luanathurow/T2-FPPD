...
type GameService struct {
	mu        sync.Mutex
	players   map[int]shared.PlayerState // estado atual dos jogadores
	processed map[int]int                // controle de comandos processados (exactly-once)
	nextID    int
}

// Registra um novo jogador e retorna seu ID
func (g *GameService) RegisterPlayer(name *string, reply *shared.PlayerState) error {
	g.mu.Lock()
	defer g.mu.Unlock()
...
func (g *GameService) UpdatePlayer(state *shared.PlayerState, reply *bool) error {
	g.mu.Lock()
	defer g.mu.Unlock()
...
func (g *GameService) GetGameState(_ *struct{}, reply *map[int]shared.PlayerState) error {
	g.mu.Lock()
	defer g.mu.Unlock()

	gameCopy := make(map[int]shared.PlayerState)
...
func mainServer() {
	...
	fmt.Println("Servidor RPC do jogo iniciado na porta :5002")
